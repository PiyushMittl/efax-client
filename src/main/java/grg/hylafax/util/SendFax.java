...
// $Id: SendFax.java,v 1.8 2007/02/21 00:07:49 sjardine Exp $
//
 ...
 * <PRE>
 * 
 * -h <host> specifiy server hostname -u <user> user to login to the server with
 * -v verbose mode -d <number> specify a destination FAX <number> -f <sender>
 * user <sender> as the identity of the FAX sender -k <time> kill the job if it
 * doesn't complete after the indicated <time> -t <tries> make no more than
 * <tries> attempts to deliver the FAX -T <dials> maximum number of <dials> to
 * attempt for each job -D enable delivery notification -R enable delivery and
 * retry notification -N disable delivery and retry notification -P <pri> assign
 * the <pri> priority to the job (default: 127) -l use low resolution (98
 * lines/inch) -m use medium resolution (196 lines/inch) -s <size> specify the
 * symbolic page <size> (legal, us-let, a3, a4, etc.)
 * 
 * </PRE>
 * 
 * <P>
 ...
 * gnu.getopt (java-getopt) can be found at <a
 * href="http://www.urbanophile.com/arenn/">http://www.urbanophile.com/arenn/</a>
 */
	...
	public static void main(String arguments[]) {
		...
		while ((short) (opt = (char) g.getopt()) != -1) {
			...
			case 'P':
				...
				break;
			case 's':
				pagesize = Pagesize.getPagesize(g.getOptarg());
				if (pagesize == null) {
					// no good
